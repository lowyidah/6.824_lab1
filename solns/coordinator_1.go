...
func(c *Coordinator) ReadyDone(args *ReadyDoneArgs, reply *ReadyDoneReply) error {
	muCoordinator.Lock()
	c.worker_status[args.WORKER_ID] = "r"

	// fmt.Println("ReadyDoneLock")
	correct_task, ok_task := c.worker_to_task[args.WORKER_ID]
	correct_status, ok_status := c.worker_status[args.WORKER_ID]
	if ok_status && correct_status != "f" && ok_task && correct_task.id == args.TASK_ID {
